...
public class CustomViewAbove extends ViewGroup {
	...
	public void setAboveOffset(int i) {
		...
		mContent.setPadding(mContent.getPaddingLeft(), i,
	...
	private void pageScrolled(int xpos) {
		final int widthWithMargin = getHeight();
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (DEBUG)
			if (action == MotionEvent.ACTION_DOWN)
				Log.v(TAG, "Received ACTION_DOWNasdf");

		if (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP
				|| (action != MotionEvent.ACTION_DOWN && mIsUnableToDrag)) {
			endDrag();
			Log.d("slide_scroll", "action");
			return false;
		}

		switch (action) {
		case MotionEvent.ACTION_MOVE:
			Log.d("slide_scroll", "ACTION_DOWN123123123");
			determineDrag(ev);
			break;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		Log.d("slide_scroll", "ACTION_start");
		if (!mEnabled)
			return false;
		...
		case MotionEvent.ACTION_DOWN:
			Log.d("slide_scroll", "ACTION_DOWN");
			/*
			...
			break;
		case MotionEvent.ACTION_MOVE:
			Log.d("slide_scroll", "ACTION_MOVE");
			...
			Log.d("slide_scroll", "ACTION_MOVE   "+ mIsBeingDragged);
			if (mIsBeingDragged) {
		...
		case MotionEvent.ACTION_UP:
			if (mIsBeingDragged) {
				...
				final float pageOffset = (float) (scrollX - getDestScrollY(mCurItem)) / getBehindHeight();
				final int activePointerIndex = getPointerIndex(ev, mActivePointerId);
				if (mActivePointerId != INVALID_POINTER) {
					final float x = MotionEventCompat.getY(ev, activePointerIndex);
	...
	private void determineDrag(MotionEvent ev) {
		...
		Log.e("test_log", ""+yDiff );
		Log.d("test_log", ""+(isMenuOpen()?mTouchSlop/2:mTouchSlop));
		Log.v("test_log", ""+thisSlideAllowed(dy) );
		
		if (yDiff > (isMenuOpen()?mTouchSlop/2:mTouchSlop)  && thisSlideAllowed(dy)) {		
			Log.e("test_log", "success" );
			startDrag();
			mLastMotionX = x;
			mLastMotionY = y;
			setScrollingCacheEnabled(true);
