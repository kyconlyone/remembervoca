...
public class TimerTutoActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//목표시간 대비 목표 달성률(/100)
		percentage_lv = ((int) use_time) / (goal * 60000);
		...
		if (min < goal) {
			//목표시간 이하
			imageLayout.setBackground(getResources().getDrawable(
					R.drawable.timer_step1));
		} else if (min >= goal && min < (goal + 10)) {
			//목표시간 ~ 목표시간+10
			imageLayout.setBackground(getResources().getDrawable(
					R.drawable.timer_step2));
		} else if (min >= (goal + 10) && min < (goal + 30)) {
			//목표시간+10 ~ 목표시간+30
			imageLayout.setBackground(getResources().getDrawable(
					R.drawable.timer_step3));
		} else {
			//목표시간+30 이상
			imageLayout.setBackground(getResources().getDrawable(
					R.drawable.timer_step4));
		...
		if (min < mPreference.getInt(MainValue.GpreGoalTime, 15)) {
			guideMsg.setText("목표까지 " + remain_min + ":" + remain_sec
					+ "\n남았어요!");
		} else {
			guideMsg.setText("목표보다 " + remain_min.replace("-", "") + ":"
					+ remain_sec.replace("-", "") + "\n더 공부했어요!");
		}
		...
		if (min < 10) {
			...
			learnTime.setText(Integer.toString((int) min) + "분 "
					+ Integer.toString((int) sec) + "초");
			goalTimeMin.setText(Integer.toString((int) min));
		}
		...
		timerTopLayout.setVisibility(timerTop.INVISIBLE);
		//애니메이션 종료 전까지 팝업창 안보이게
		popupLayout.setVisibility(popupLayout.GONE);
				...
				toTop.setAnimationListener(new AnimationListener() {
					...
					public void onAnimationStart(Animation animation) {
						...
						if (hit_tuto == true) {
							...
							//메인단어장에서 불렀을 때
							HitTutoActivity.percent_view
									.setVisibility(HitTutoActivity.percent_view.INVISIBLE);
						}
					}
				...
				toBottom.setAnimationListener(new AnimationListener() {
					...
					public void onAnimationEnd(Animation animation) {
						...
						if (hit_tuto == true) {
							...
							//단어장에서 실행했을 때
							HitTutoActivity.hit_tuto_layout
									.updateViewLayout(HitTutoActivity.percent_view,
											mRootLayoutParams);

							HitTutoActivity.percent_view
									.setVisibility(HitTutoActivity.percent_view.VISIBLE);

							HitTutoActivity.restart_flag = true;
							HitTutoActivity.c_timer.start();
						}
						timerLayout.setVisibility(View.GONE);
						animationFlag = true;

						TimerTutoActivity.this.finish();
	...
	public void onStart()
	{
		...
		FlurryAgent.logEvent("TimerClickActivity", articleParams);
		// your code
		//	HitTutoActivity.writeLog("[모든단어 암기완료 시작,FinishStudyFragment,1]\r\n");
	...
	{
		super.onStop();
		FlurryAgent.endTimedEvent("TimerClickActivity");
		...
		// your code
		//	HitTutoActivity.writeLog("[모든단어 암기완료 끝,FinishStudyFragment,1,{Start:"+articleParams.get("Start")+",End:"+articleParams.get("End")+"}]\r\n");
	}
}
